...
import { QuillDeltaToHtmlAsyncConverter } from './../src/QuillDeltaToHtmlAsyncConverter';
import { callWhenXEqualYAsync } from './_helper';

import { delta1 } from './data/delta1';
import { GroupType, ListType } from './../src/value-types';
import { encodeHtml } from './../src/funcs-html';

describe('QuillDeltaToHtmlAsyncConverter', function () {
  describe('constructor()', function () {
    ...
    it('should instantiate return proper html', async function () {
      var qdc = new QuillDeltaToHtmlAsyncConverter(delta1.ops, {
        classPrefix: 'noz',
      });
      var html = await qdc.convert();
      assert.equal(html, delta1.html);
    });

    it('should set default inline styles for `inlineStyles: true`', async function () {
      var qdc = new QuillDeltaToHtmlAsyncConverter(hugeOps, {
        inlineStyles: true,
        customCssStyles: (op) => {
    ...
    it('should set default inline styles when `inlineStyles` is a truthy non-object', async function () {
      var qdc = new QuillDeltaToHtmlAsyncConverter(hugeOps, {
        inlineStyles: 1,
      } as any);
      var html = await qdc.convert();
    ...
    it('should allow setting inline styles', async function () {
      var qdc = new QuillDeltaToHtmlAsyncConverter(hugeOps, {
        inlineStyles: {
          size: {
            huge: 'font-size: 6em',
  ...
  describe('convert()', function () {
    ...
    it('should render html', async function () {
      var qdc = new QuillDeltaToHtmlAsyncConverter(ops2);

      var html = await qdc.convert();
      assert.equal(html.indexOf('<pre>this is code') > -1, true, html);
    });

    it('should render mention', async function () {
      let ops = [
        {
          insert: 'mention',
          attributes: {
      ...
      var qdc = new QuillDeltaToHtmlAsyncConverter(ops);
      var html = await qdc.convert();
      assert.equal(
        html,
        [
      ...
      var qdc = new QuillDeltaToHtmlAsyncConverter([
        {
          insert: 'mention',
          attributes: {
            mentions: true,
    ...
    it('should render links with rels', async function () {
      var ops = [
        {
          attributes: {
            link: '#',
      ...
      qdc = new QuillDeltaToHtmlAsyncConverter(ops);
      html = await qdc.convert();
      assert.equal(
        html,
        '<p><a href="#" target="_blank" rel="nofollow noopener">external link</a><a href="#" target="_blank">internal link</a></p>'
      );
    });
    it('should render image and image links', async function () {
      let ops = [
      ...
      let qdc = new QuillDeltaToHtmlAsyncConverter(ops);
      let html = await qdc.convert();
      assert.equal(
        html,
        [
          '<p>',
    ...
    it('should open and close list tags', async function () {
      var ops4 = [
        { insert: 'mr\n' },
        { insert: 'hello' },
      ...
      var qdc = new QuillDeltaToHtmlAsyncConverter(ops4);
      var html = await qdc.convert();
    ...
    it('should render as separate paragraphs', async function () {
    ...
    it('should create checked/unchecked lists', async function () {
      var ops4 = [
        { insert: 'hello' },
    ...
    it('should wrap positional styles in right tag', async function () {
      var ops4 = [
        { insert: 'mr' },
    ...
    it('should render target attr correctly', async () => {
      let ops = [
      ...
      let qdc = new QuillDeltaToHtmlAsyncConverter(ops, { linkTarget: '' });
      let html = await qdc.convert();
      assert.equal(
        html,
        [
      ...
      qdc = new QuillDeltaToHtmlAsyncConverter(ops, { linkTarget: '_top' });
      html = await qdc.convert();
      assert.equal(
        html,
        [
    ...
    it('should convert using custom url sanitizer', async () => {
      let ops = [
    ...
    it('should render singe cell table', async () => {
      let ops = [
        {
          insert: 'cell',
        },
    ...
    it('should render filled table', async () => {
      let ops = [
        {
          insert: '11',
        },
  ...
  describe('custom types', () => {
    it(`should return empty string if renderer not defined for custom blot`, async () => {
      let ops = [{ insert: { customstuff: 'my val' } }];
      let qdc = new QuillDeltaToHtmlAsyncConverter(ops);
      assert.equal(await qdc.convert(), '<p></p>');
    });
    it('should render custom insert types with given renderer', async () => {
      let ops = [
    ...
    it('should render custom insert types as blocks if renderAsBlock is specified', async () => {
      let ops = [
        { insert: 'hello ' },
    ...
    it('should render custom insert types in code blocks with given renderer', async () => {
      let ops = [
        { insert: { colonizer: ':' } },
        { insert: '\n', attributes: { 'code-block': true } },
        { insert: 'code1' },
      ...
      let qdc = new QuillDeltaToHtmlAsyncConverter(ops.slice(0, 2));
      qdc.renderCustomWith(renderer);
      assert.equal(await qdc.convert(), '<pre>:</pre>');

      qdc = new QuillDeltaToHtmlAsyncConverter(ops);
      qdc.renderCustomWith(renderer);
      assert.equal(await qdc.convert(), '<pre>:\ncode1\n:</pre>');
      ...
      qdc.renderCustomWith(renderer);
      assert.equal(await qdc.convert(), '<code>:\ncode1\n:</code>');
    });

    it('should render custom insert types in headers with given renderer', async () => {
      let ops = [
        { insert: { colonizer: ':' } },
        { insert: '\n', attributes: { header: 1 } },
        { insert: 'hello' },
      ...
      let qdc = new QuillDeltaToHtmlAsyncConverter(ops.slice(0, 2));
      qdc.renderCustomWith(renderer);
      assert.equal(await qdc.convert(), '<h1>:</h1>');

      qdc = new QuillDeltaToHtmlAsyncConverter(ops);
      qdc.renderCustomWith(renderer);
      assert.equal(await qdc.convert(), '<h1>:<br/>hello<br/>:</h1>');
    });
  });
  ...
  describe(' prepare data before inline and block renders', function () {
    ...
    describe('renderInlines()', function () {
      it('should render inlines', async function () {
        var qdc = new QuillDeltaToHtmlAsyncConverter([]);
        var inlines = await qdc._renderInlines(ops);
        assert.equal(
          inlines,
          ['<p>Hello', '<em> my </em><br/> name is joey</p>'].join('')
        );

        qdc = new QuillDeltaToHtmlAsyncConverter([], { paragraphTag: 'div' });
        var inlines = await qdc._renderInlines(ops);
        assert.equal(
          inlines,
        ...
        qdc = new QuillDeltaToHtmlAsyncConverter([], { paragraphTag: '' });
        var inlines = await qdc._renderInlines(ops);
        assert.equal(inlines, 'Hello<em> my </em><br/> name is joey');
      });

      it('should render inlines custom tag', async function () {
        var qdc = new QuillDeltaToHtmlAsyncConverter([], {
          customTag: (format) => {
            if (format === 'italic') {
              return 'i';
      ...
      it('should render plain new line string', async function () {
        var ops = [new DeltaInsertOp('\n')];
        var qdc = new QuillDeltaToHtmlAsyncConverter([]);
        assert.equal(await qdc._renderInlines(ops), '<p><br/></p>');
      });

      it('should render styled new line string', async function () {
        var ops = [new DeltaInsertOp('\n', { font: 'arial' })];
        var qdc = new QuillDeltaToHtmlAsyncConverter([]);
        assert.equal(await qdc._renderInlines(ops), '<p><br/></p>');

        var qdc = new QuillDeltaToHtmlAsyncConverter([], { paragraphTag: '' });
        assert.equal(await qdc._renderInlines(ops), '<br/>');
      });

      it('should render when first line is new line', async function () {
        var ops = [new DeltaInsertOp('\n'), new DeltaInsertOp('aa')];
        var qdc = new QuillDeltaToHtmlAsyncConverter([]);
        assert.equal(await qdc._renderInlines(ops), '<p><br/>aa</p>');
      });

      it('should render when last line is new line', async function () {
        var ops = [new DeltaInsertOp('aa'), new DeltaInsertOp('\n')];
        var qdc = new QuillDeltaToHtmlAsyncConverter([]);
        assert.equal(await qdc._renderInlines(ops), '<p>aa</p>');
      });

      it('should render mixed lines', async function () {
        ...
        var qdc = new QuillDeltaToHtmlAsyncConverter([]);
        assert.equal(await qdc._renderInlines(ops), '<p>aabb</p>');

        var ops0 = [nlop, ops[0], nlop, ops[1]];
        assert.equal(await qdc._renderInlines(ops0), '<p><br/>aa<br/>bb</p>');

        var ops4 = [ops[0], stylednlop, stylednlop, stylednlop, ops[1]];
        assert.equal(
          await qdc._renderInlines(ops4),
    ...
    describe('renderBlock()', function () {
      ...
      it('should render container block', async function () {
        var qdc = new QuillDeltaToHtmlAsyncConverter([]);
        var blockhtml = await qdc._renderBlock(op, [inlineop]);
        assert.equal(
          blockhtml,
        ...
        var qdc = new QuillDeltaToHtmlAsyncConverter([]);
        var blockhtml = await qdc._renderBlock(op, []);
        assert.equal(
          blockhtml,
      ...
      it('should correctly render code block', async function () {
        let ops = [
          {
            insert: 'line 1',
          },
        ...
        qdc = new QuillDeltaToHtmlAsyncConverter([ops[0], ops[1]]);
        html = await qdc.convert();
        assert.equal(html, '<pre>line 1</pre>');
      });
    });

    it('should correctly render custom text block', async function () {
      let ops = [
        {
          insert: 'line 1',
        },
    ...
    describe('before n after renders()', function () {
      ...
      it('should call before/after render callbacks ', async function () {
        let status = { x: 0, y: 8 };
        ...
        await callWhenXEqualYAsync(status);
      });

      it(`should call before render with block grouptype for align indent and direction`, async () => {
        let ops = [
          { insert: 'align' },
      ...
      it('should use my custom html if I return from before call back', async function () {
        var c = new QuillDeltaToHtmlAsyncConverter([
          { insert: { video: 'http' } },
          { insert: 'aa' },
        ]);
        c.beforeRender(() => {
        ...
        var v = await c.convert();
        assert.ok(v.indexOf('<my custom') > -1);
      });

      it('should register and use callbacks if they are functions', async function () {
        var c = new QuillDeltaToHtmlAsyncConverter([
          { insert: { video: 'http' } },
          { insert: 'aa' },
        ]);
