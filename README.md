...
|`linkTarget`| string \| ((url)=>string\|undefined) | '_blank' | Specifies target for all links; use `''` (empty string) to not generate `target` attribute. This can be overridden by an individual link op by specifiying the `target` with a value in the respective op's attributes. If a function is provided, then it is called on each URL.|
